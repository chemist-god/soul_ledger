...
import { Calendar, Download, TrendingUp, ArrowUpRight, BarChart3, PieChart as PieChartIcon, Activity } from "lucide-react"
...
import { COLORS, MONTHLY_GOALS, GOAL_CATEGORY_BREAKDOWN, MOCK_STATS, TOAST } from "@/lib/gen-variable"
        ...
        <div className="flex flex-col w-full min-h-screen">
            <header className={`flex h-16 shrink-0 items-center justify-between border-b border-black/5 px-4 sm:px-6 bg-[${COLORS.background}] sticky top-0 z-20`}>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <Button
                        onClick={() => toast(TOAST.exportStarted.title, { description: TOAST.exportStarted.description })}
            ...
            <main className={`flex-1 p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8 bg-[${COLORS.background}]`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-black">Goal Analytics</h1>
                        <p className="text-black/60 text-[14px] sm:text-[15px]">Your progress, streaks, and staking performance over time.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                    <Card className="rounded-3xl border-none shadow-sm bg-black text-white p-6 relative overflow-hidden flex flex-col justify-between min-h-[160px]">
                        ...
                        <div className="relative z-10 flex justify-between items-start">
                            <span className="text-white/60 font-medium text-[14px]">Total Stake Earnings</span>
                        ...
                        <div className="relative z-10 mt-4">
                            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">{MOCK_STATS.totalEarned}</h2>
                            <div className="flex items-center gap-2 mt-2">
                                <span className={`flex items-center text-[${COLORS.primary}] font-semibold text-[13px] bg-[${COLORS.primary}]/10 px-2 py-0.5 rounded-md`}>
                                    <TrendingUp className="mr-1 h-3 w-3" /> {MOCK_STATS.totalEarnedChange}
                                </span>
                    ...
                    <Card className="rounded-3xl border-black/5 shadow-sm bg-white p-6 relative overflow-hidden flex flex-col justify-between min-h-[160px]">
                        <div className="relative z-10 flex justify-between items-start">
                            <span className="text-black/50 font-medium text-[14px]">Avg. Stake Amount</span>
                            <div className="h-8 w-8 rounded-full bg-black/5 flex items-center justify-center">
                                <BarChart3 className="h-4 w-4 text-black/60" />
                            </div>
                        </div>
                        <div className="relative z-10 mt-4">
                            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-black">{MOCK_STATS.avgStakeAmount}</h2>
                            <div className="flex items-center gap-2 mt-2">
                                <span className="flex items-center text-green-600 font-semibold text-[13px] bg-green-50 px-2 py-0.5 rounded-md">
                    ...
                    <Card className={`rounded-3xl border-black/5 shadow-sm bg-[${COLORS.primary}] p-6 relative overflow-hidden flex flex-col justify-between min-h-[160px]`}>
                        <div className="relative z-10 flex justify-between items-start">
                            <span className="text-black/60 font-medium text-[14px]">Community Stakers</span>
                        ...
                        <div className="relative z-10 mt-4">
                            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-black">{MOCK_STATS.communityStakers}</h2>
                            <div className="flex items-center gap-2 mt-2">
                                <span className="flex items-center text-black font-semibold text-[13px] bg-white/40 px-2 py-0.5 rounded-md">
                                    <TrendingUp className="mr-1 h-3 w-3" /> +2 new
                                </span>
                                <span className="text-black/50 text-[13px]">backing your goals</span>
                            </div>
                        </div>
                    </Card>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <Card className="xl:col-span-2 rounded-3xl border-black/5 shadow-sm bg-white overflow-hidden">
                        <CardHeader className="p-6 pb-2 border-b border-black/5">
                            ...
                            <CardDescription className="text-[14px] text-black/50">Monthly breakdown of goals completed vs. failed.</CardDescription>
                        </CardHeader>
                        <CardContent className="p-6">
                            <div className="h-[300px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={MONTHLY_GOALS} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                        <defs>
                                                ...
                                                if (active && payload && payload.length) {
                                                    return (
                                                        <div className="bg-black text-white text-xs font-semibold px-3 py-2 rounded-lg shadow-lg">
                                                            ...
                                                            <div className="text-[14px]">{payload[0].value} completed</div>
                                                        </div>
                                                    );
                                                }
                                                return null;
                    ...
                    <Card className="rounded-3xl border-black/5 shadow-sm bg-white overflow-hidden flex flex-col">
                        <CardHeader className="p-6 pb-2 border-b border-black/5">
                            <CardTitle className="text-lg font-bold text-black">Goal Categories</CardTitle>
                            <CardDescription className="text-[14px] text-black/50">Which areas you&apos;re most focused on.</CardDescription>
                        </CardHeader>
                        <CardContent className="p-6 flex-1 flex flex-col justify-center">
                            <div className="h-[220px] w-full relative">
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie
                                            ...
                                            stroke="none"
                                        >
                                            {GOAL_CATEGORY_BREAKDOWN.map((entry, index) => (
                                ...
                                <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                                    <span className="text-sm font-semibold text-black/50">Goals</span>
                                    <span className="text-xl font-bold text-black">{MOCK_STATS.goalsCompletedAllTime + MOCK_STATS.goalsFailedAllTime}</span>
                                </div>
                            </div>

                            <div className="mt-8 space-y-3">
                                {GOAL_CATEGORY_BREAKDOWN.map((category, i) => (
                                    <div key={i} className="flex items-center justify-between text-sm">
                                        ...
                                        <span className="font-semibold text-black">{category.value} goals</span>
                                    </div>
                                ))}
                            </div>
                        </CardContent>
